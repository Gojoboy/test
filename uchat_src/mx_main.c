...
if (newitem == NULL) { write(1, " row n1 \n", 10); sleep(1); }
//sleep(1);
write(1, " user create \n", 15);
    if ( true ) { // test case 1
...
write(1, " user create5\n", 15);
                ...
                m1->m_form = 1;
                m1->m_m_id = 0;
write(1, " user creat52\n", 15);
                c2->c_messagearr[c2->c_messagearr_count] = m1;
                c2->c_messagearr_count++;
write(1, " user creat53\n", 15);
                t_message* m2 = (t_message*)malloc( sizeof(t_message) );
                m2->m_id = 32;
                m2->u_id = 3;
                ...
                m2->m_form = 1;
                m2->m_m_id = 0;
write(1, " user creat54\n", 15);
                c2->c_messagearr[c2->c_messagearr_count] = m2;
                c2->c_messagearr_count++;
write(1, " user create6\n", 15);
                t_message* m3 = (t_message*)malloc( sizeof(t_message) );
                m3->m_id = 33;
                m3->u_id = 2;
                ...
                m3->m_form = 1;
                m3->m_m_id = 0;
                c2->c_messagearr[c2->c_messagearr_count] = m3;
                c2->c_messagearr_count++;
                ...
                m4->m_form = 1;
                m4->m_m_id = 0;
                c2->c_messagearr[c2->c_messagearr_count] = m4;
                c2->c_messagearr_count++;
            }
        ...
        t_chat* c3 = (t_chat*)malloc( sizeof(t_chat) );
        {
            ...
            c3->c_messagearr_size = 21;
            c3->c_messagearr = (t_message**)malloc( sizeof(t_message*) * c3->c_messagearr_size );
...
write(1, " chat add2\n", 12);
        mx_add_chat( user->u_chatarr[0]);
        mx_add_chat( user->u_chatarr[1]);
        mx_add_chat( user->u_chatarr[2]);
    }
...
write(1, " 1 ", 4);
    ...
    /*gtk_widget_hide( GTK_WIDGET(app->chats_list_page->profile_overlay->overlay_background) ); // chat list overlay (profile)
write(1, " 2 ", 4);
    gtk_widget_hide( GTK_WIDGET(app->chats_list_page->profile_overlay->overlay_main) ); // chat list overlay (profile)
write(1, " 3 ", 4);

    gtk_widget_hide( GTK_WIDGET(app->chats_list_page->add_chat_overlay->overlay_background) ); // chat list overlay (add chat)
write(1, " 4 ", 4);
    gtk_widget_hide( GTK_WIDGET(app->chats_list_page->add_chat_overlay->overlay_main) ); // chat list overlay (add chat)
write(1, " 5 ", 4);

    gtk_widget_hide( GTK_WIDGET(app->chats_list_page->search_scroller) ); // chat list search overlay
write(1, " 6 ", 4);

    gtk_widget_hide( GTK_WIDGET(app->chat_page->profile_overlay->overlay_background) ); // chat overlay (profile)
write(1, " 7 ", 4);
    gtk_widget_hide( GTK_WIDGET(app->chat_page->profile_overlay->overlay_main) ); // chat overlay (profile)*/
